...
def main(argv):
  ...
  outlist = ["string_hex.anal", "array_hex.anal", "else_hex.anal", "f100_hex.anal"]
...
#  anal_f_mul = open("tyN+f_hex.anal")
  ...
  for i in range(n_of_data):
    ...
    if not mode:
      ...
      anal_f.write("type "+ str(ty)+ "["+str(N+1)+ "]")
    ...
    while len(write_str) < size:
      ...
      anal_f.write("  +  type func["+str(N+1)+"]")
      while len(write_str) < size and line:
        write_str.append(line.pop(0))
