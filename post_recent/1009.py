...
def main(argv):
  ...
  out_f = open(cur_path+"/output/df_1009test_"+str(size)+".list", "w")
  anal_f = open(cur_path+"/output/df_1009test_"+str(size)+".anal", "w")
  ...
  for i in range(n_of_data):
    ...
    if not mode:
      ...
      while (True):
        ...
        spl2 = hexf[ty][N][:-1].split() # remove '\n'
        spl = []
        for sp in spl2:
          spl.append(int(sp, 16))

        # test: data alter for kby
        if len(spl) < 4:
          continue
        if spl[3]<=143.5:
          if spl[0]<=223.5:
